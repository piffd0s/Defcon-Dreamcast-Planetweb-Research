#!/usr/bin/env python3
"""
dreamcast_doom.py -- one-command DEF CON demo:
    Dreamcast PlanetWeb browser  ->  email MIME overflow  ->  native SH-4 DOOM.

    sudo python3 dreamcast_doom.py            # COLD (default): fresh render, live staging
    sudo python3 dreamcast_doom.py --warm     # restore the pre-armed golden/armed snapshot
    sudo python3 dreamcast_doom.py --bake      # (re)build the golden state
    sudo python3 dreamcast_doom.py --arm       # (re)build the one-click armed state

Cold is the default because a warm savestate restore hits the browser's black-on-restore
repaint quirk (boots to a black screen); cold renders fresh and is the reliable demo path.
...
def _iface_inet(iface):
    ...
    DHCP leases, so a hotspot/tethered/static address (e.g. 192.0.0.2) needs the ifconfig fallback."""
    try:
...
def kill_stale():
    ...
    for name in ("dns_redirect.py", "mail_poc.py", "eden_mitm.py", "mail_mitm.py", "dreamcast_doom.py"):
        reap(name, name)
...
class RSP:
    def __init__(self, host="127.0.0.1", port=GDB_PORT, timeout=8):
        ...
        # Flycast halts the guest the instant we connect and sends an unsolicited stop-reply
        # ($T05..#xx). Drain it now so it can't desync the first real command's response --
        # this is exactly what hung staging once stage() became the first thing to touch the stub.
        time.sleep(0.3)
...
def halt(r):
    ...
    try:
        while r.s.recv(4096): pass            # drain the stop-reply(ies)
    except Exception: pass
    r.s.settimeout(8)                          # KEEP a finite timeout -- never block forever
                                               # (was setblocking(True), which blocked indefinitely)
...
def launch_flycast(ip, bake=False, force_armed=False, save_on_quit=False, cold=False):
    ...
    for attempt in range(1, FLYCAST_ATTEMPTS + 1):
        ...
        if ok:
            # "Game ID" means reios started. Do NOT probe the SH-4 over the GDB stub here:
            # sending it a halt (\x03) mid-boot pauses the guest and is exactly what stalls
            # the boot. Accept the boot if the process stays alive -- the clean baseline flash
            # boots healthy, and the stub is used later (staging), on purpose, once we're online.
            time.sleep(2)
            if p.poll() is None:
                if attempt > 1: say("(booted on attempt %d)" % attempt)
                return p
            ok = False; why = "Flycast exited right after reios"
...
def bake_state(ip):
    ...
    say("")
    say("now every  sudo python3 dreamcast_doom.py  warm-boots straight to this state.")

# ------------------------------------------------------------------ main
def main():
    bake = "--bake" in sys.argv
    arm  = "--arm" in sys.argv
    # COLD (fresh render) is the DEFAULT: a warm savestate restore hits the browser's
    # black-on-restore repaint quirk (boots to a black screen). Pass --warm to opt into the
    # pre-armed snapshot (instant, but only when the restore repaints). --cold still accepted.
    cold = "--warm" not in sys.argv
    have_armed = os.path.exists(ARMED) and not cold
    if os.geteuid() != 0:
        warn("needs root for :53/:80/:110 -- run:  sudo python3 dreamcast_doom.py"); sys.exit(1)
    ...
    say("mode      %s" % ("COLD -- fresh render (default)" if cold
                          else "WARM -- restore pre-armed snapshot (--warm)"))
    say("golden    %s" % ("present (use --warm)" if have_golden() else "ABSENT -- run once with --bake"))
    say("armed     %s" % ("present -- use --warm for one-click" if os.path.exists(ARMED)
                          else "absent -- build with --arm"))
    if ip == "127.0.0.1":
    ...
    # Do NOT connect to the GDB stub here. Flycast halts the guest the instant a client
    # connects, which freezes the browser mid-boot -- this is the "debugging pauses the boot"
    # hang. We touch the stub for the first time only at staging (stage(), below), long after
    # you've taken the browser online. The stub stays enabled the whole time; we just leave it
    # alone until it's actually needed.
    say("emulator up (browser booting -- GDB left untouched until staging)")
